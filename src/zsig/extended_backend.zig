...
pub const ExtendedKeypair = struct {
    ...
    pub fn verify(self: *const ExtendedKeypair, message: []const u8, signature: ExtendedSignature) bool {
        ...
        return switch (self.algorithm) {
            ...
            .ml_dsa_65 => {
                const keys = self.ml_dsa_keys.?;
                const is_valid = zcrypto.pq.ml_dsa.ML_DSA_65.KeyPair.verify(keys.public_key, message, signature.ml_dsa_sig.?) catch return false;
                return is_valid;
            },
            .hybrid_x25519_ml_kem => {
...
pub fn verifyWithPublicKey(message: []const u8, signature: ExtendedSignature, public_key_bytes: []const u8, algorithm: Algorithm) bool {
    return switch (algorithm) {
        .ed25519 => {
        ...
        .ml_dsa_65 => {
            ...
            const is_valid = zcrypto.pq.ml_dsa.ML_DSA_65.KeyPair.verify(public_key, message, signature.ml_dsa_sig.?) catch return false;
            return is_valid;
        },
        .hybrid_x25519_ml_kem => {
