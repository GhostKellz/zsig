...
pub const Keypair = struct {
    ...
    pub fn signWithContext(self: *const Keypair, message: []const u8, context: []const u8) ![64]u8 {
        switch (self.algorithm) {
            .ed25519 => {
                // Use hash with context to incorporate context into signature
                const hashed = hashWithContext(context, message);
                const kp = crypto.KeyPair{
                    .private_key = self.secret_key,
                    .public_key = self.public_key,
                };
                return kp.sign(&hashed);
            },
        }
    }
    ...
    pub fn publicKeyHex(self: *const Keypair, allocator: std.mem.Allocator) ![]u8 {
        return std.fmt.allocPrint(allocator, "{s}", .{std.fmt.bytesToHex(&self.public_key, .lower)});
    }
    
    /// Get secret key as hex string
    pub fn secretKeyHex(self: *const Keypair, allocator: std.mem.Allocator) ![]u8 {
        return std.fmt.allocPrint(allocator, "{s}", .{std.fmt.bytesToHex(&self.secret_key, .lower)});
    }
};
...
pub const Signature = struct {
    ...
    pub fn toHex(self: *const Signature, allocator: std.mem.Allocator) ![]u8 {
        return std.fmt.allocPrint(allocator, "{s}", .{std.fmt.bytesToHex(&self.bytes, .lower)});
    }
...
/// Verify a signature
pub fn verify(message: []const u8, signature: [64]u8, public_key: [32]u8, algorithm: Algorithm) bool {
