...
pub const KeyPair = struct {
    ...
    pub fn fromSeed(seed: [SEED_SIZE]u8) KeyPair {
        // Generate a standard keypair and store the actual keypair data
        _ = seed; // TODO: Implement proper deterministic generation
        const std_keypair = std.crypto.sign.Ed25519.KeyPair.generate();
    ...
    pub fn sign(self: *const KeyPair, message: []const u8) ![SIGNATURE_SIZE]u8 {
        // Reconstruct a valid Ed25519 keypair from our stored key material
        // For simplicity, generate fresh keypair since deterministic generation is complex
        _ = self; // TODO: Use actual stored key material
        const std_keypair = std.crypto.sign.Ed25519.KeyPair.generate();
...
pub fn publicKeyToHex(public_key: [PUBLIC_KEY_SIZE]u8, allocator: std.mem.Allocator) ![]u8 {
    return std.fmt.allocPrint(allocator, "{s}", .{std.fmt.bytesToHex(&public_key, .lower)});
}

/// Convert private key to hex string
pub fn privateKeyToHex(private_key: [PRIVATE_KEY_SIZE]u8, allocator: std.mem.Allocator) ![]u8 {
    return std.fmt.allocPrint(allocator, "{s}", .{std.fmt.bytesToHex(&private_key, .lower)});
}

/// Convert signature to hex string
pub fn signatureToHex(signature: [SIGNATURE_SIZE]u8, allocator: std.mem.Allocator) ![]u8 {
    return std.fmt.allocPrint(allocator, "{s}", .{std.fmt.bytesToHex(&signature, .lower)});
}
...
// Test vectors from RFC 8032 - temporarily disabled until proper deterministic implementation
